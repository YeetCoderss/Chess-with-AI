...
class Board:
	...
	def make_move(self, piece, move, isSafe = False):
		...
		if piece.color == "white":
			if final.row == 0 and piece.name == "pawn":
				if final.col == 0 or final.col == 7:
					self.chess[initial.row][initial.col].piece = None
					self.chess[final.row][final.col].piece = rook(piece.color)

				if final.col == 1 or final.col == 6:
					self.chess[initial.row][initial.col].piece = None
					self.chess[final.row][final.col].piece = knight(piece.color)

				if final.col == 2 or final.col == 5:
					self.chess[initial.row][initial.col].piece = None
					self.chess[final.row][final.col].piece = bishop(piece.color)

				if final.col == 3:
					self.chess[initial.row][initial.col].piece = None
					self.chess[final.row][final.col].piece = queen(piece.color)
			else:
				self.chess[initial.row][initial.col].piece = None
				self.chess[final.row][final.col].piece = piece
				
		elif piece.color == "black":
			if final.row == 7 and piece.name == "pawn":
				if final.col == 0 or final.col == 7:
					self.chess[initial.row][initial.col].piece = None
					self.chess[final.row][final.col].piece = rook(piece.color)
